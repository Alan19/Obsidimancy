...
public class ObsidianShard extends Item {
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        ...
        if (overworldAttuneProgress != 0) {
            pTooltipComponents.add(Component.translatable("obsidimancy.overworld_attunement_message", String.format("%.2f", overworldAttuneProgress / 3 * 100) + "%").withStyle(ChatFormatting.BLUE));
        }
        if (netherAttuneProgress != 0) {
            pTooltipComponents.add(Component.translatable("obsidimancy.nether_attunement_message", String.format("%.2f", netherAttuneProgress / 3 * 100) + "%").withStyle(ChatFormatting.RED));
        }
        if (endAttuneProgress != 0) {
            pTooltipComponents.add(Component.translatable("obsidimancy.end_attunement_message", String.format("%.2f", endAttuneProgress / 3 * 100) + "%").withStyle(ChatFormatting.AQUA));
        }
    }
}
